...
const concatStrings = function (firstString, secondString) {
  const splitFirstString = firstString.split("");
  const splitSecondString = secondString.split("");
  const newArray1 = splitFirstString.slice(0, 2);
  const newArray2 = splitSecondString.slice(splitSecondString.length - 3);
  return newArray1.join("").toUpperCase() + newArray2.join("").toUpperCase();
};

console.log(concatStrings("Prima stringa di prova", "Seconda stringa di test"));
...
const numbers = [2, 5, 6, 1, 23, 44, 78, 3];

const findEvenNum = numbers.filter((num) => num % 2 === 0);
console.log("Dati il seguente array di numeri:", numbers);
console.log("Sono stati estrapolati solo i numeri PARI:", findEvenNum);
...
const sumTotal = findEvenNum.reduce((total, nums) => total + nums);
...
const increaseNum = numbers.map((number) => number * 2);
...
const arrayString = ["Ciao", "mi", "chiamo", "Emanuele"];

const stringLength = arrayString.map((len) => len.length);
...
const numbersDispUntil99 = [];
numbersUntil99.forEach((dispNum) => {
  if (dispNum % 2 !== 0) {
    numbersDispUntil99.push(dispNum);
  }
});
console.log(numbersDispUntil99);
